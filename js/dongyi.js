...
window.dongyi = window.dj = {
	...
	pay:function(json){
		gM.ready(function(){		
			...
			if(json.url == undefined || json.url == ''){
				url = "http://api.dongyi.me/alipay.php?name="+json.name+"&dyappid="+json.appid+"&dyappkey="+json.appkey+"&packagename="+dongyi.getRuntime('appid')+"&payid=";
			}else{
				url = json.url + '?payid=';
			}
	...
	sendIdCode:function(jsonData){
		...
		dongyi.get({
			url:'http://api.dongyixueyuan.com/sms_service/index.php',
			data:{
				code:jsonData.code,
